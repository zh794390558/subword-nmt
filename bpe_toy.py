...
def get_stats(vocab):
  ...
  for word, freq in vocab.items():
    symbols = word.split()
    print('word', word, 'symbols', symbols)
...
def merge_vocab(pair, v_in):
  v_out = {}
  print('before bigram_pattern', ' '.join(pair))
  bigram_pattern = re.escape(' '.join(pair))
  print('bigram_pattern', bigram_pattern)
  p = re.compile(r'(?<!\S)' + bigram_pattern + r'(?!\S)')
  for word in v_in:
    w_out = p.sub(''.join(pair), word)
    print('merge_vocab', 'word', word, 'w_out', w_out)
    v_out[w_out] = v_in[word]
  return v_out

vocab = {'l o w</w>' : 5, 'l o w e r</w>' : 2,
         'n e w e s t</w>' : 6, 'w i d e s t</w>' : 3}
print('raw vocab:', vocab)
...
for i in range(num_merges):
  ...
  try:
    for key in pairs:
        print('iter paris', 'value', pairs.get(key), 'key',  key)
